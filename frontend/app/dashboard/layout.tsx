    ...
    return (
        <div className="flex min-h-screen bg-[#fafafa]">
            ...
            <div className="ml-64 flex-1 flex flex-col min-h-screen">
                {/* Top Header */}
                <header className="h-16 bg-white dark:bg-zinc-900 border-b border-slate-200 dark:border-zinc-800 px-8 flex items-center justify-between sticky top-0 z-40">
                    <div className="flex items-center gap-4">
                        <h2 className="text-lg font-bold text-slate-800 dark:text-white">{title}</h2>
                        <span className="px-2 py-0.5 bg-slate-100 dark:bg-zinc-800 text-[11px] font-bold text-slate-500 rounded uppercase tracking-tighter">{subtitle}</span>
                    </div>
                    <div className="flex items-center gap-6">
                        ...
                        <div className="flex items-center gap-3 cursor-pointer group">
                            <div className="text-right">
                                <p className="text-xs font-bold text-slate-800 dark:text-white group-hover:text-[#0059b2] transition-colors">{isManager ? 'Site Manager' : 'Deepak Sharma'}</p>
                                <p className="text-[10px] text-slate-500 font-medium">{isManager ? 'Project Lead' : 'Head of EHS'}</p>
                            </div>
